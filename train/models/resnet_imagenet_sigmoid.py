...
from models.base_model_sigmoid import BaseModel
...
class ResNet(BaseModel):
    ...
    def penultimate(self, x, all_features=False):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)     #x.shape = 256,512
...
def _resnet(arch, block, layers,pretrained, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load("/home/mars/workspace/ljh_workspace/Contrastive_CXR-3D/resnet18-f37072fd.pth")
        model.load_state_dict(checkpoint,strict=False)   #r3d_18 가중치(pth) 불러오고 가중치에 없는건 뛰어넘는다 (strict=False)
    return model
