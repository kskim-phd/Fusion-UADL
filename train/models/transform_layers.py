...
class Rotation1(nn.Module):
    def __init__(self, max_range = 4):
        super(Rotation1, self).__init__()
        self.max_range = max_range
        self.prob = 0.5
    ...
    def forward(self, input, aug_index=None):
        ...
        if aug_index is None:
            ...
            output = torch.rot90(input, aug_index, (2,4))  #x,z 돌림 >> 결국 Y축을 기준으로 돌린것 
            ...
            output = torch.rot90(input, aug_index, (2,4))

        return output

class Rotation2(nn.Module):
    def __init__(self, max_range = 4):
    ...
    def forward(self, input, aug_index=None):
        ...
        if aug_index is None:
            ...
            output = torch.rot90(input, aug_index, (3,4))  # y,z 돌림 >> 결국 Z축을 기준으로 돌린것 (원래는 x축기준 돌린거지만 우리 데이터는 x축 자리가 depth임 즉 z축!)
...
class Rotation_2D(nn.Module):
    def __init__(self, max_range = 4):
        super(Rotation_2D, self).__init__()
        self.max_range = max_range
        self.prob = 0.5
