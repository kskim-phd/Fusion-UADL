...
for patient in tqdm(patient_list):
    ...
    gp = nib.load(root_current+'/G-UADL_score/'+patient.split('/')[-1].replace('nii.gz','npy.nii.gz')).get_fdata()
    whole_imgs=[]
    whole_masks=[]
    whole_annotation=[]
    ...
    if test_img.shape[0] < 200:
        ...
        for idx_slice in range(test_img.shape[1]):
                ...
                whole_cam_gp.append(cv2.resize(gp[:,idx_slice,:],(512,200),interpolation=cv2.INTER_NEAREST))
        ...
        gp=np.array(whole_cam_gp).transpose((1,0,2))[::-1,:,:]
    ...
    lp = np.load(root_current+'/L-UADL_score/'+patient.split('/')[-1].replace('.nii.gz','.npy'))

    nonzero_lp = lp[np.nonzero(lp)].ravel()
    lp[lp<lp[np.nonzero(lp)].min()]=lp[np.nonzero(lp)].min()

    score_p6 = np.percentile(nonzero_lp, 6) 

    lp[lp<score_p6]=score_p6

    nonzero_gp = gp[np.nonzero(gp)].ravel()
    gp[gp<gp[np.nonzero(gp)].min()]=gp[np.nonzero(gp)].min()

    score_p94 = np.percentile(nonzero_gp, 94) 

    gp[gp>score_p94]=score_p94
    

    gp=(gp-gp.min())/(gp.max()-gp.min())   #minmax normalize
    ...
    score_max = np.percentile(nonzero_cam, 94) 
    score_min = np.percentile(nonzero_cam, 82)

    cam[cam>score_max]=score_max
    cam[cam<score_min]=score_min

   
    cam=(cam-cam.min())/(cam.max()-cam.min())
    

    cam_minmax=np.array(cam*255,dtype='uint8')
    ...
    # print('pred_coordinates: {}, {}, {}'.format(x_pred,y_pred,z_pred))
    ...
    save_path = root_current+'/CAM_results/Fusion-UADL/'
    os.makedirs(save_path, exist_ok=True)
    out = cv2.VideoWriter(save_path\
    +test_img_dir.split('/')[-1].replace('.nii.gz','')+'_fusion.avi',cv2.VideoWriter_fourcc(*'DIVX'), 1, size)  #환자당 CAM video 저장.
