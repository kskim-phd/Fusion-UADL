...
for patient in tqdm(patient_list):
    ...
    cam = nib.load(root_current+'/G-UADL_score/'+patient.split('/')[-1].replace('nii.gz','npy.nii.gz')).get_fdata()
    whole_imgs=[]
    whole_masks=[]
    whole_annotation=[]
    ...
    if test_img.shape[0] < 200:
        ...
        for idx_slice in range(test_img.shape[1]):
                ...
                whole_cam.append(cv2.resize(cam[:,idx_slice,:],(512,200),interpolation=cv2.INTER_NEAREST))
        ...
        cam=np.array(whole_cam).transpose((1,0,2))[::-1,:,:]
    ...
    score_p94 = np.percentile(nonzero_cam, 94) 
    score_p18 = np.percentile(nonzero_cam, 82)

    cam[cam>score_p94]=score_p94
    cam[cam<score_p18]=score_p18

    cam=(cam-cam.min())/(cam.max()-cam.min())   #minmax normalize
    ...
    save_path = root_current+'/CAM_results/G-UADL/'
    os.makedirs(save_path, exist_ok=True)
    out = cv2.VideoWriter(save_path\
    +test_img_dir.split('/')[-1].replace('.nii.gz','')+'_GP.avi',cv2.VideoWriter_fourcc(*'DIVX'), 1, size)  #환자당 CAM video 저장.
